...
export class MpesaClient {
	...
	constructor(config: ApiConfig) {
		...
		if (!this.apiConfig.bearerToken)
			this.apiConfig.bearerToken = MpesaClient.generateBearerToken(
	...
	public async queryTransactionStatus(
		...
		const url: string = this.getUrl(
			'/ipg/v1x/queryTransactionStatus' + this.getStringParams(params),
			'18353',
		);
	...
	public async reversalTransaction(
		params: ReversalParams,
	): Promise<SyncReversal> {
	 ...
	 * @returns
	 */
	public async queryCustomerName(
		params: CustomerNameParams,
	): Promise<SyncCustomerName | AsyncResponse> {
		params = this.addServiceCodeIntoBody(params);
		const url: string = this.getUrl(
			'/ipg/v1x/queryCustomerName' + this.getStringParams(params),
			'19323',
		);
		const res: any = await this.get(url);
		return await res.json();
	...
	private addServiceCodeIntoBody(
		...
		type: 'service' | 'partycode' = 'service',
	): RequestParams {
		if (type === 'service')
	...
	private getReplyMpesa(
		url: string,
	): (params: AsyncResponseAfterRequest) => Promise<void> {
